...
|--------|-----------|-------------|
| `Buffer` | `pub struct Buffer` | Copy-on-open file buffer. Public field: `path: PathBuf`. Internal: `original: Vec<u8>`, `edits: HashMap<usize, u8>`. |
...
| `set` | `pub fn set(&mut self, offset: usize, value: u8)` | Sets a byte in the edit overlay. If value matches original, removes the edit (restoring original). |
